...
export async function processDocument(docId, { force = false } = {}) {
  ...
  setProcessing(docId, { status: 'processing', error: null });

  const isNewContent = force || doc.processing?.contentHash !== hash;
  if (isNewContent) {
    deletePassagesForDoc(docId);
    const extracted = extractPassages(doc.html);
    for (const p of extracted) {
      createPassage({ docId, order: p.order, location: p.location, html: p.html, text: p.text });
    }
  }

  let passages = passagesForDoc(docId);
  ...
  if (!isLlmConfigured()) {
    ...
    for (const passage of stillAmbiguous) {
      updatePassage(passage.id, { matches: [], matchStatus: 'matched' });
    }
  } else {
    for (const batch of batchArray(stillAmbiguous)) {
      try {
        ...
        for (const passage of batch) {
          updatePassage(passage.id, { matches: results.get(passage.id) || [], matchStatus: 'matched' });
        }
      } catch (err) {
        // Leave this batch (and any not yet attempted) as 'pending' so the
        // next call to processDocument only retries what's left undone.
        setProcessing(docId, { status: 'error', error: err.message, contentHash: hash });
        return;
      }
    }
  }

  passages = passagesForDoc(docId);
